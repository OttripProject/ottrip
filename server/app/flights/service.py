...
class FlightService:
    ...
    async def create(self, *, flight_data: FlightCreate) -> int:
        ...
        if not flight_data.segments or len(flight_data.segments) < 1:
            raise HTTPException(
                status_code=400, detail="세그먼트는 최소 1개 이상이어야 합니다."
            )

        # 시간 순서대로 정렬 (departure_time 기준)
        sorted_segments = sorted(
            flight_data.segments, key=lambda seg: seg.departure_time
        )
    ...
    async def read_flight(self, *, flight_id: int) -> FlightRead:
        ...
        if flight.plan.owner_id != self.current_user.id:
            ...
            if not is_shared:
                raise HTTPException(
                    status_code=403, detail="항공편 조회 권한이 없습니다."
                )
    ...
    async def read_flights_by_plan(self, *, plan_id: int) -> list[FlightRead]:
        ...
        if not has_permission:
            raise HTTPException(
                status_code=403, detail="해당 항공편에 대한 조회 권한이 없습니다."
            )
    ...
    async def update(self, *, flight_id: int, update_data: FlightUpdate) -> None:
        ...
        if update_data.segments is not None:
            ...
            existing_segments = await self.flight_repository.find_segments_by_flight(
                flight_id=flight.id
            )
            existing_segment_ids = {seg.id for seg in existing_segments}

            new_segment_ids = {
                seg.id for seg in update_data.segments if seg.id is not None
            }
            ...
            # 시간 순서대로 정렬 (departure_time 기준)
            sorted_segments = sorted(
                update_data.segments, key=lambda seg: seg.departure_time
            )

            for idx, seg_data in enumerate(sorted_segments, start=1):
                if seg_data.id and seg_data.id in existing_segment_ids:
                    existing_seg = next(
                        (s for s in existing_segments if s.id == seg_data.id), None
                    )
                    if existing_seg:
        ...
        if update_data.expense:
            if flight.expense:
                ...
                existing_expense = await self.expense_repository.find_by_flight_id(
                    flight_id=flight.id
                )
    ...
    async def add_segment(self, *, data: FlightSegmentCreate) -> FlightSegmentRead:
        ...
        if flight.plan.owner_id != self.current_user.id:
            ...
            if not is_editor:
                raise HTTPException(
                    status_code=403, detail="세그먼트 추가 권한이 없습니다."
                )
        ...
        all_segments = await self.flight_repository.find_segments_by_flight(
            flight_id=flight.id
        )
        ...
        # 업데이트된 segment 반환
        updated_segment = await self.flight_repository.find_segment_by_id(
            segment_id=created.id
        )
        return FlightSegmentRead.model_validate(updated_segment)

    async def update_segment(
        self, *, segment_id: int, data: FlightSegmentUpdate
    ) -> FlightSegmentRead:
        ...
        if flight.plan.owner_id != self.current_user.id:
            ...
            if not is_editor:
                raise HTTPException(
                    status_code=403, detail="세그먼트 수정 권한이 없습니다."
                )
    ...
    async def delete_segment(self, *, segment_id: int) -> None:
        ...
        if flight.plan.owner_id != self.current_user.id:
            ...
            if not is_editor:
                raise HTTPException(
                    status_code=403, detail="세그먼트 삭제 권한이 없습니다."
                )
        await self.flight_repository.soft_delete_segment(segment_id=segment_id)
