...
export default function LoginScreen() {
  ...
  useEffect(() => {
    if (Platform.OS === "web") {
      ...
      if (hash && hash.includes("invite=")) {
        ...
        if (inviteToken) {
          try {
            window.localStorage.setItem("pendingInviteToken", inviteToken);
          } catch {}
      ...
      if (Platform.OS === "web") {
        ...
        try {
          ...
          if (error.code === "SIGN_IN_CANCELLED") {
            ...
            showMessage(
              "알림",
              "로그인에 실패했습니다: " + (error.message || "알 수 없는 오류"),
            );
            setIsLoading(false);
          }
        }
  ...
  const onAppleSignInWeb = async () => {
    ...
    try {
      ...
      const message =
        e instanceof Error ? e.message : "Apple 로그인에 실패했습니다.";
  ...
  const onGuestStart = async () => {
    ...
    try {
      ...
      showMessage(
        "알림",
        "비회원으로 시작할 수 없습니다. 잠시 후 다시 시도해주세요.",
      );
    } finally {
      setIsLoading(false);
    }
    ...
    try {
      ...
      try {
        ...
        if (error.response?.data?.detail) {
          ...
          errorMessage =
            "Dev 라우터를 찾을 수 없습니다. 서버가 개발 모드로 실행되고 있는지 확인해주세요.";
      ...
      showMessage(
        "알림",
        `테스트 로그인 중 알림가 발생했습니다: ${userErrorMessage}`,
      );
      setIsLoading(false);
    }
  };
